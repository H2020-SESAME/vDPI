...
byt_sent_viber = 0, byt_sent_spot = 0, byt_sent_fb = 0;
...
void my_sigalarm(int sig) {
  ...
  curl_easy_setopt(curl, CURLOPT_URL, "http://localhost:8086/write?db=dpi");
  //printf("%s\n", strcat(strcat("http://", influxDBurl) , "/write?db=flows") );
  //printf("%s\n", concat(concat("http://", influxDBurl), "/write?db=flows"));
  //curl_easy_setopt(curl, CURLOPT_URL, concat(influxDBurl, "/write?db=dpi"));
  //curl_easy_setopt(curl, CURLOPT_URL, "http://localhost:8086/write?db=flows");
  ...
  double dbyt_sent_bittor = 0, dbyt_sent_http, dbyt_sent_skype = 0;   
    byt_sent = (byt_sent*8);    
    byt_sent_bittor = (byt_sent_bittor*8);    
    byt_sent_http = (byt_sent_http*8);    
    byt_sent_skype = (byt_sent_skype*8);    
    byt_sent_twit = (byt_sent_twit*8);    
    byt_sent_youtube = (byt_sent_youtube*8);    
    byt_sent_google = (byt_sent_google*8);    
    byt_sent_dropb = (byt_sent_dropb*8);    
    byt_sent_apple = (byt_sent_apple*8);    
    byt_sent_icloud = (byt_sent_icloud*8);    
    byt_sent_viber = (byt_sent_viber*8);    
    byt_sent_dns = (byt_sent_dns*8);
	byt_sent_fb = (byt_sent_fb*8);

  sprintf(postthis, "mbits_packets_all,name=all,geohash=swbbjvx8wcrn value=%u \n mbits_packets_bittorrent,name=all value=%u \n \
  mbits_packets_http,name=http,geohash=swbbjvx8wcrn value=%u  \n mbits_packets_skype,name=all value=%u \n mbits_packets_twitter,name=all value=%u \n \
  mbits_packets_youtube,name=youtube,geohash=swbbjvx8wcrn value=%u  \n mbits_packets_google,name=google,geohash=swbbjvx8wcrn value=%u \n mbits_packets_dropbox,name=all value=%u \n \
  mbits_packets_apple,name=all value=%u  \n mbits_packets_icloud,name=all value=%u \n mbits_packets_viber,name=viber,geohash=swbbjvx8wcrn value=%u \n mbits_packets_dns,name=all value=%u \n \
  mbits_packets_fb,name=fb,geohash=swbbjvx8wcrn value=%u"
  ,byt_sent,byt_sent_bittor,byt_sent_http,byt_sent_skype,byt_sent_twit,byt_sent_youtube
  ,byt_sent_google,byt_sent_dropb,byt_sent_apple,byt_sent_icloud,byt_sent_viber,byt_sent_dns, byt_sent_fb);
  //printf("%s\n",postthis);
...
static unsigned int packet_processing(u_int16_t thread_id,
  ...
  if(flow->detection_completed){
	proto_app = ndpi_get_proto_name(ndpi_thread_info[thread_id].ndpi_struct, flow->detected_protocol.protocol);
    return(0);
  }
...
static void pcap_packet_callback(u_char *args,
 ...
 iph_check:
  ...
  packet_processing(thread_id, time, vlan_id, iph, iph6,
	...
	num_sent++;
    byt_sent += header->caplen;
	
	if(proto_app != NULL){
    if(strcmp(proto_app, "HTTP") == 0){
        byt_sent_http += header->caplen;
      }
      else if(strcmp(proto_app, "BitTorrent")==0 ){
        byt_sent_bittor += header->caplen;
      }
      else if(strcmp(proto_app,"Facebook")==0) byt_sent_fb += header->caplen;
      else if(strcmp(proto_app,"YouTube")==0){
        byt_sent_youtube += header->caplen;
      } 
      else if(strcmp(proto_app,"Skype")==0){
        
        byt_sent_skype += header->caplen;
      }
      else if(strcmp(proto_app,"Twitter")==0){
        
        byt_sent_twit += header->caplen;
      }
      else if(strcmp(proto_app,"Google")==0){
        
        byt_sent_google += header->caplen;
      }
      else if(strcmp(proto_app,"DropBox")==0){
        
        byt_sent_dropb += header->caplen;
      }
      else if(strcmp(proto_app,"Apple")==0){
        
        byt_sent_apple += header->caplen;
      }
      else if(strcmp(proto_app,"AppleiCloud")==0){
        
        byt_sent_icloud += header->caplen;
      }
      else if(strcmp(proto_app,"Viber")==0){
        
        byt_sent_viber += header->caplen;
      }
	  else if(strcmp(proto_app,"Facebook")==0){
        
        byt_sent_fb += header->caplen;
      }
    }
}
